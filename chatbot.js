...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (robot) {
        robot.addEventListener('mouseenter', () => {
            ...
            robot.querySelector('.robot-arm.arm-left').style.animation = 'wave 1s infinite';
            robot.querySelector('.robot-arm.arm-right').style.animation = 'wave 1s infinite';

            // Change screen content
            const screenContent = robot.querySelector('.screen-content');
            screenContent.innerHTML = `
                        <span class="screen-line">User detected!</span>
                        <span class="screen-line">Analyzing intent...</span>
                        <span class="screen-line">How can I help?</span>
                        <span class="screen-line">> Hello Human!</span>
                    `;
        });

        robot.addEventListener('mouseleave', () => {
            robot.style.animation = 'float 6s ease-in-out infinite';
            robot.querySelector('.robot-arm.arm-left').style.animation = 'wave 4s infinite';
            robot.querySelector('.robot-arm.arm-right').style.animation = 'wave 4s infinite 0.5s';

            // Reset screen content
            const screenContent = robot.querySelector('.screen-content');
            screenContent.innerHTML = `
                        <span class="screen-line">Initializing AI...</span>
                        <span class="screen-line">Loading NLP models...</span>
                        <span class="screen-line">Ready to assist!</span>
                        <span class="screen-line">> Hello World!</span>
                    `;
        });
    }
});
